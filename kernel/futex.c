...
dword_t sys_futex(addr_t uaddr, dword_t op, dword_t val, addr_t timeout_or_val2, addr_t uaddr2, dword_t val3) {
    ...
    // Bitset ops are treated as matching every waiter, which only adds
    // spurious wakeups. Their timeout is absolute, so make it relative.
    if (cmd == FUTEX_WAIT_BITSET_ || cmd == FUTEX_WAKE_BITSET_) {
        if (val3 == 0)
            return _EINVAL;
        if (cmd == FUTEX_WAIT_BITSET_ && timeout_or_val2) {
            struct timespec now;
        ...
        cmd = cmd == FUTEX_WAIT_BITSET_ ? FUTEX_WAIT_ : FUTEX_WAKE_;
    }
    switch (cmd) {
        case FUTEX_WAIT_:
